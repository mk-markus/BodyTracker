...
{
    public partial class DBEntryOxygenSaturationViewModel : ObservableObject
    { /// <summary>
      ...
      /// </remarks>
        ...
        /// <summary>
        /// Gets or sets the collection of oxygen saturation records currently displayed in the UI.
        /// Uses <see cref="ObservableCollection{T}"/> to automatically notify the UI 
        /// of additions, removals, or list clears.
        /// </summary>
        [ObservableProperty] private ObservableCollection<SamsungOxygenSaturationModel> oxygenSaturationDatas = new();

        /// <summary>
        /// Gets or sets the currently selected oxygen saturation record from the list.
        /// Nullable, as no record may be selected.
        /// </summary>
        [ObservableProperty] private SamsungOxygenSaturationModel? selectedData;
        ...
        /// <summary>
        /// Gets the command that initiates the deletion of the currently selected oxygen saturation record.
        ...
        /// <summary>
        /// Gets the command responsible for refreshing the oxygen saturation history from the database.
        ...
        /// </remarks>
        public DBEntryOxygenSaturationViewModel(DatabaseService db)
        {
            databaseService = db;
        ...
        /// <summary>
        /// Synchronizes the local collection with the oxygen saturation data stored in the database.
        /// </summary>
        /// <returns>A task that represents the asynchronous reload operation.</returns>
        /// <remarks>
        /// The process involves retrieving the selected person's identifier and fetching their latest oxygen saturation records.
        /// </remarks>
        private async Task ReloadAsync()
        {
            var pid = AppState.SelectedPersonId;

            await GetCurrentOxygenSaturation(pid);
        }

        /// <summary>
        /// Asynchronously retrieves the current oxygen saturation records for the specified person identifier from the database and updates the observable collection.
        /// </summary>
        /// <param name="pid">The unique person identifier used to query current oxygen saturation entries.</param>
        /// <returns>A task representing the asynchronous operation.</returns>
        private async Task GetCurrentOxygenSaturation(int pid)
        {
            var all = await databaseService.GetSpO2SqlAsync(pid);

            OxygenSaturationDatas.Clear();
            foreach (var m in all)
            {
                OxygenSaturationDatas.Add(m);
            }
            OnPropertyChanged(nameof(OxygenSaturationDatas));
        }

        /// <summary>
        /// Asynchronously retrieves the initial oxygen saturation records for the specified person identifier from the database and updates the observable collection.
        /// </summary>
        /// <param name="pid">The unique person identifier used to query initial oxygen saturation entries.</param>
        /// <returns>A task representing the asynchronous operation.</returns>
        private async Task GetInitialOxygenSaturation(int pid)
        {
        ...
        /// </summary>
        /// <returns>
        /// <c>true</c> if a oxygen saturation record is selected in the UI (<see cref="SelectedData"/> is not null); 
        /// otherwise, <c>false</c>.
        /// </returns>
        /// <remarks>
        ...
        /// </remarks>
        partial void OnSelectedDataChanged(SamsungOxygenSaturationModel? value)
        {
            (CommandDelete as AsyncRelayCommand)?.NotifyCanExecuteChanged();
        }

        /// <summary>
        /// Asynchronously deletes the currently selected oxygen saturation record from the database.
        /// It removes the entry based on its unique oxygen saturation identifier.
        /// </summary>
        ...
        private async Task DeleteSelectedAsync()
        {
            try
            {
                ...
                if (SelectedData.OxygenSaturationID.HasValue) await databaseService.GetSpO2DeleteSqlAsync(SelectedData.OxygenSaturationID.Value);
                await ReloadAsync();
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Asynchronously updates an existing oxygen saturation record or inserts a new one into the database.
        /// This method acts as a wrapper for the data access layer, ensuring that all oxygen saturation data 
        /// is persisted before triggering a full UI refresh.
        /// </summary>
        /// <param name="personId">The unique identifier of the person to whom the oxygen saturation belongs.</param>
        /// <param name="row">The view model containing the oxygen saturation data to be synchronized.</param>
        ...
        /// </remarks>
        public async Task UpdateRowDataAsync(int personId, SamsungOxygenSaturationModel row)
        {
            return;
        }

        /// <summary>
        /// Handles the <see cref="DataGrid.RowEditEnding"/> event to persist modified oxygen saturation data to the database.
        ...
        private async Task DataGrid_RowEditEnding(DataGridRowEditEndingEventArgs e)
        {
            ...
            if (e.Row.Item is not SamsungOxygenSaturationModel editedRow) return;
