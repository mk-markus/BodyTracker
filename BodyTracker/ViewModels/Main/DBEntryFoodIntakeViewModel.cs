...
{
    public partial class DBEntryFoodIntakeViewModel : ObservableObject
    {  
        /// <summary>
       ...
       /// </remarks>
        ...
        /// <summary>
        /// Gets or sets the collection of food intake records currently displayed in the UI.
        /// Uses <see cref="ObservableCollection{T}"/> to automatically notify the UI 
        /// of additions, removals, or list clears.
        /// </summary>
        [ObservableProperty] private ObservableCollection<SamsungFoodIntakeModel> foodIntakeDatas = new();

        /// <summary>
        /// Gets or sets the currently selected food intake record from the list.
        /// Nullable, as no record may be selected.
        /// </summary>
        [ObservableProperty] private SamsungFoodIntakeModel? selectedData;
        ...
        /// <summary>
        /// Gets the command that initiates the deletion of the currently selected food intake record.
        ...
        /// <summary>
        /// Gets the command responsible for refreshing the food intake history from the database.
        ...
        /// </remarks>
        public DBEntryFoodIntakeViewModel(DatabaseService db)
        {
            databaseService = db;
        ...
        /// <summary>
        /// Synchronizes the local collection with the food intake data stored in the database.
        /// </summary>
        /// <returns>A task that represents the asynchronous reload operation.</returns>
        /// <remarks>
        /// The process involves retrieving the selected person's identifier and fetching their latest food intake records.
        /// </remarks>
        private async Task ReloadAsync()
        {
            var pid = AppState.SelectedPersonId;

            await GetCurrentFoodIntakes(pid);
        }

        /// <summary>
        /// Asynchronously retrieves the current food intake records for the specified person identifier from the database and updates the observable collection.
        /// </summary>
        /// <param name="pid">The unique person identifier used to query current food intake entries.</param>
        /// <returns>A task representing the asynchronous operation.</returns>
        private async Task GetCurrentFoodIntakes(int pid)
        {
            var all = await databaseService.GetSamsungFoodIntakeSqlAsync(pid);

            FoodIntakeDatas.Clear();
            foreach (var m in all)
            {
                FoodIntakeDatas.Add(m);
            }
            OnPropertyChanged(nameof(FoodIntakeDatas));
        }

        /// <summary>
        /// Asynchronously retrieves the initial food intake records for the specified person identifier from the database and updates the observable collection.
        /// </summary>
        /// <param name="pid">The unique person identifier used to query initial food intake entries.</param>
        /// <returns>A task representing the asynchronous operation.</returns>
        ...
        /// </summary>
        /// <returns>
        /// <c>true</c> if a food intake record is selected in the UI (<see cref="SelectedData"/> is not null); 
        /// otherwise, <c>false</c>.
        /// </returns>
        /// <remarks>
        ...
        /// </remarks>
        partial void OnSelectedDataChanged(SamsungFoodIntakeModel? value)
        {
            (CommandDelete as AsyncRelayCommand)?.NotifyCanExecuteChanged();
        }

        /// <summary>
        /// Asynchronously deletes the currently selected food intake record from the database.
        /// It removes the entry based on its unique food intake identifier.
        /// </summary>
        ...
        private async Task DeleteSelectedAsync()
        {
            try
            {
                ...
                if (SelectedData.FoodIntakeID.HasValue) await databaseService.GetFoodIntakeDeleteSqlAsync(SelectedData.FoodIntakeID.Value);
                await ReloadAsync();
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Asynchronously updates an existing food intake record or inserts a new one into the database.
        /// This method acts as a wrapper for the data access layer, ensuring that all food intake data 
        /// is persisted before triggering a full UI refresh.
        /// </summary>
        /// <param name="personId">The unique identifier of the person to whom the food intake belongs.</param>
        /// <param name="row">The view model containing the food intake data to be synchronized.</param>
        ...
        /// </remarks>
        public async Task UpdateRowDataAsync(int personId, SamsungFoodIntakeModel row)
        {
            return;
        }

        /// <summary>
        /// Handles the <see cref="DataGrid.RowEditEnding"/> event to persist modified food intake data to the database.
