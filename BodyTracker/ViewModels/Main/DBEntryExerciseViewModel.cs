...
{
    public partial class DBEntryExerciseViewModel : ObservableObject
    {  /// <summary>
       ...
       /// </remarks>
        ...
        /// <summary>
        /// Gets or sets the collection of exercise records currently displayed in the UI.
        /// Uses <see cref="ObservableCollection{T}"/> to automatically notify the UI 
        /// of additions, removals, or list clears.
        /// </summary>
        [ObservableProperty] private ObservableCollection<SamsungExerciseModel> exerciseDatas = new();

        /// <summary>
        /// Gets or sets the currently selected exercise record from the list.
        /// Nullable, as no record may be selected.
        /// </summary>
        [ObservableProperty] private SamsungExerciseModel? selectedData;
        ...
        /// <summary>
        /// Gets the command that initiates the deletion of the currently selected exercise record.
        ...
        /// <summary>
        /// Gets the command responsible for refreshing the exercise history from the database.
        ...
        /// </remarks>
        public DBEntryExerciseViewModel(DatabaseService db)
        {
            databaseService = db;
        ...
        /// <summary>
        /// Synchronizes the local collection with the exercise data stored in the database.
        /// </summary>
        /// <returns>A task that represents the asynchronous reload operation.</returns>
        /// <remarks>
        /// The process involves retrieving the selected person's identifier and fetching their latest exercise records.
        /// </remarks>
        ...
        /// <summary>
        /// Asynchronously retrieves the current exercise records for the specified person identifier from the database and updates the observable collection.
        /// </summary>
        /// <param name="pid">The unique person identifier used to query current exercise entries.</param>
        /// <returns>A task representing the asynchronous operation.</returns>
        private async Task GetCurrentFoodIntakes(int pid)
        {
            var all = await databaseService.GetSamsungExerciseAsync(pid);
            ...
            OnPropertyChanged(nameof(ExerciseDatas));
        }

        /// <summary>
        /// Asynchronously retrieves the initial exercise records for the specified person identifier from the database and updates the observable collection.
        /// </summary>
        /// <param name="pid">The unique person identifier used to query initial exercise entries.</param>
        /// <returns>A task representing the asynchronous operation.</returns>
        ...
        /// </summary>
        /// <returns>
        /// <c>true</c> if a exercise record is selected in the UI (<see cref="SelectedData"/> is not null); 
        /// otherwise, <c>false</c>.
        /// </returns>
        /// <remarks>
        ...
        /// </remarks>
        partial void OnSelectedDataChanged(SamsungExerciseModel? value)
        {
            (CommandDelete as AsyncRelayCommand)?.NotifyCanExecuteChanged();
        }

        /// <summary>
        /// Asynchronously deletes the currently selected exercise record from the database.
        /// It removes the entry based on its unique exercise identifier.
        /// </summary>
        ...
        private async Task DeleteSelectedAsync()
        {
            try
            {
                ...
                if (SelectedData.ExerciseID.HasValue) await databaseService.GetExerciseDeleteSqlAsync(SelectedData.ExerciseID.Value);
                await ReloadAsync();
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Asynchronously updates an existing exercise record or inserts a new one into the database.
        /// This method acts as a wrapper for the data access layer, ensuring that all exercise data 
        /// is persisted before triggering a full UI refresh.
        /// </summary>
        /// <param name="personId">The unique identifier of the person to whom the exercise belongs.</param>
        /// <param name="row">The view model containing the exercise data to be synchronized.</param>
        ...
        /// <summary>
        /// Handles the <see cref="DataGrid.RowEditEnding"/> event to persist modified exercise data to the database.
