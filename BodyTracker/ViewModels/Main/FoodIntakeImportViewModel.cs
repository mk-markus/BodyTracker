...
{
    public partial class FoodIntakeImportViewModel : ObservableObject
    {
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets the collection of <see cref="SamsungFoodIntakeModel"/> objects currently loaded in the view.
        /// </summary>
        ...
        /// </summary>
        private string searchTerm = "food_intake";
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="FoodIntakeImportViewModel"/> class.
        /// </summary>
        ...
        public FoodIntakeImportViewModel(DatabaseService db, string path)
        {
            databaseService = db;
            searchPath = path;
        ...
        private async Task OpenAsync()
        {
            ...
            try
            {
                ...
                var list = await SamsungDataCsvExtractor.ParseFoodIntakeAsync(
                    dlg.FileName,
                    progress,
                    new Progress<string>(text => ProgressText = text));

                FoodIntakeDatas = new ObservableCollection<SamsungFoodIntakeModel>(list);
        ...
        /// <summary>
        /// Initiates the asynchronous upload of the current <see cref="FoodIntakeDatas"/> collection 
        ...
        {
            if (FoodIntakeDatas == null || FoodIntakeDatas.Count == 0)
            {
            ...
            try
            {
                ...
                await databaseService.UpsertFoodIntakeSqlAsync(pid, FoodIntakeDatas, progress);

                ProgressText = $"Uploaded {FoodIntakeDatas.Count} Datas)";
            }
            catch (Exception ex)
            {
                GeneralInfoMessage = $"Error during upload: {ex.Message}";
            }
            finally
            {
                IsLoading = false;
