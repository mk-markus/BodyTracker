...
namespace BodyTracker.Services
{
    ...
    public partial class DatabaseService : ObservableObject, IDisposable
    {
        ...
        public async Task<bool> MeasurementExistsAsync(DateTime dateToCheck, int PersonId)
        {
            var SqlServerConnection = await OpenConnectionAsync();
            try
            {
        ...
        /// <returns>A task representing the asynchronous operation.</returns>
        public async Task AddBodyMetricAsync(BodyMetricModel bodyMetric)
        {
            await using var sqlServerConnection = await OpenConnectionAsync();
            if (!IsConnected) return;
            try
        ...
        /// <returns>A task representing the asynchronous operation.</returns>
        public async Task AddBodyDimensionAsync(BodyDimensionsModel bodyDemension)
        {
            await using var sqlServerConnection = await OpenConnectionAsync();
            if (!IsConnected) return;
        ...
        public async Task<bool> AddFoodIntakeAsync(int PersonId, ICollection<SamsungFoodIntakeModel> foodIntakeList, IProgress<double>? progress = null)
        {
            await using var sqlServerConnection = await OpenConnectionAsync();
        ...
        /// <param name="progress">An optional progress indicator that reports the percentage of the insertion process.</param>
        /// <returns>Returns 'true' if the operation completes successfully.</returns>
        public async Task<bool> SyncFoodIntakeAsync(int personId, ICollection<SamsungFoodIntakeModel> foodIntakeList,
        ...
        public async Task<bool> AddExerciseAsync(int PersonId,
                                                  ...
                                                  IProgress<double>? progress = null)
        {
            ...
            try
            {
                ...
                foreach (var exercise in exerciseList)
                {
                    ...
                    sqlCommand.Parameters.AddWithValue("@Count", (object?)exercise.Count ?? DBNull.Value);
        ...
        /// <returns>True if successful; otherwise false.</returns>
        public async Task<bool> SyncExerciseAsync(int personId, ICollection<SamsungExerciseModel> exerciseList,
                                                  IProgress<(double value, string prgressText)>? progress = null)
        {
            await using var sqlServerConnection = await OpenConnectionAsync();
            ...
            try
            {
                ...
                await using var cmd = new MySqlCommand(DatabaseCommands.GetExerciseExistenceCheckSql(),
                                                        sqlServerConnection);
                ...
                foreach (var item in updates)
                {
                    ...
                    command.Parameters.AddWithValue("@ClientDataVer", (object?)item.ClientDataVer ?? DBNull.Value);
                    command.Parameters.AddWithValue("@Count", (object?)item.Count ?? DBNull.Value);
        ...
        public async Task<bool> AddHeartRatesAsync(int personId,
        ...
        /// <param name="progress">Optional progress reporter.</param>
        /// <returns>True if the operation succeeds; otherwise false.</returns>
        public async Task<bool> SyncHeartRatesAsync(int personId, ICollection<SamsungHeartRateModel> heartRates,
                                                    IProgress<(double value, string prgressText)>? progress = null) 
        {
            await using var sqlServerConnection = await OpenConnectionAsync();
        ...
        /// <param name="progress">An optional progress indicator that reports the percentage of the insertion process.</param>
        /// <returns>Returns 'true' if the operation completes successfully.</returns>
        public async Task<bool> AddStepTrendAsync(int PersonId, IEnumerable<SamsungStepTrendModel> stepDailyTrendList, IProgress<double>? progress = null)
        {
            await using var sqlServerConnection = await OpenConnectionAsync();
        ...
        /// <param name="progress">An optional progress indicator that reports the percentage of the insertion process.</param>
        /// <returns>Returns 'true' if the operation completes successfully.</returns>
        public async Task<bool> SyncStepTrendAsync(int personId, ICollection<SamsungStepTrendModel> stepDailyTrendList,
                                                    IProgress<(double value, string prgressText)>? progress = null)
        {
            await using var sqlServerConnection = await OpenConnectionAsync();
            ...
            try
            {
                ...
                // Compare the stepTrendList List with the existitng values in the database tbl_StepDailyTrend. If the value exist then we update the entrie.
                // Is the value new then we insert
                // it in the table with bulk inserting.
                ...
                if (inserts.Count > 0)
                {
                    ...
                    var bulkCopy = new MySqlBulkCopy(sqlServerConnection)
                    {
                        DestinationTableName = "tbl_StepDailyTrend",
                        NotifyAfter = 1000
                    };
        ...
        /// <param name="progress">An optional progress indicator that reports the percentage of the insertion process.</param>
        /// <returns>Returns 'true' if the operation completes successfully.</returns>
        public async Task<bool> AddOxygenSaturationAsync(int PersonId, IEnumerable<SamsungOxygenSaturationModel> oxygenSaturation,
                                                         IProgress<(double value, string prgressText)>? progress = null)
        {
            await using var sqlServerConnection = await OpenConnectionAsync();
        ...
        /// <param name="progress">An optional progress indicator that reports the percentage of the insertion process.</param>
        /// <returns>Returns 'true' if the operation completes successfully.</returns>
        public async Task<bool> SyncOxygenSaturationAsync(int personId, ICollection<SamsungOxygenSaturationModel> oxygenSaturation,
                                                    IProgress<(double value, string prgressText)>? progress = null)
        {
            await using var sqlServerConnection = await OpenConnectionAsync();
                                                   ...
                                                   IProgress<double>? progress = null)
        {
            ...
            try
            {
                ...
                {
                    await using var sqlCommand = new MySqlCommand(DatabaseCommands.GetWorkoutUpsertSql(),
                                                                  sqlServerConnection,
                                                                  (MySqlTransaction)tx);
        ...
        /// <param name="progress">Optional progress reporting.</param>
        /// <returns>True if successful.</returns>
        public async Task<bool> SyncWorkoutLogAsync(int personId, ICollection<HeavyAppCSVModel> stepDailyTrendList,
                                                    IProgress<(double value, string prgressText)>? progress = null)
        {
            await using var sqlServerConnection = await OpenConnectionAsync();
            ...
            try
            {
                ...
                await using var cmd = new MySqlCommand(DatabaseCommands.GetWorkoutExistenceCheckSql(),
                                                        sqlServerConnection);
                ...
                await using (var reader = await cmd.ExecuteReaderAsync())
                {
                    while (await reader.ReadAsync())
                    {
                        existingEntries.Add(
                            (
                                reader.IsDBNull("start_time")
                                    ? null
                                    : reader.GetDateTime("start_time"),

                                reader.IsDBNull("end_time")
                                    ? null
                                    : reader.GetDateTime("end_time"),

                                reader.IsDBNull("exercise_title")
                                    ? null
                                    : reader.GetString("exercise_title"),

                                reader.IsDBNull("set_index")
                                    ? null
                                    : reader.GetInt32("set_index")
                            ),
                            (
                                reader.IsDBNull("weight_current")
                                    ? null
                                    : reader.GetDouble("weight_current"),

                                reader.IsDBNull("reps_current")
                                    ? null
                                    : reader.GetInt32("reps_current"),

                                reader.IsDBNull("distance_current")
                                    ? null
                                    : reader.GetDouble("distance_current"),

                                reader.IsDBNull("duration_sec_current")
                                    ? null
                                    : reader.GetInt32("duration_sec_current"),

                                reader.IsDBNull("rpe_current")
                                    ? null
                                    : reader.GetDouble("rpe_current"),
                ...
                var inserts = new List<HeavyAppCSVModel>();
                var updates = new List<HeavyAppCSVModel>();

                // Compare the heavy app List with the existitng values in the database tbl_Heav. If the value exist then we update the entrie.
                // Is the value new then we insert
                // it in the table with bulk inserting.
                foreach (var item in stepDailyTrendList)
                {
                    ...
                    if ((item.StartTime == dbValues.StartTime) && (item.SetIndex == dbValues.SetIndex)
                        &&(dbValues.Weight != item.WeightKg
                        || dbValues.Reps != item.Reps
                        || dbValues.Distance != item.DistanceKm
                        || dbValues.DurationSec != item.DurationSeconds
                        || dbValues.Rpe != item.Rpe))
                    {
                        updates.Add(item);
                    }
                }
                ...
                if (inserts.Count > 0)
                {
                    var table = HeavyAppDataTable.CreateHeavyAppDataTable(personId, inserts);

                    var bulkCopy = new MySqlBulkCopy(sqlServerConnection)
                    {
                        DestinationTableName = "tbl_HeavyApp",
                        NotifyAfter = 1000
                    };
                    ...
                    bulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(11, "weight_current"));
                    bulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(12, "weight_initial"));
                    bulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(13, "reps_current"));
                    bulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(14, "reps_initial"));
                    bulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(15, "distance_current"));
                    bulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(16, "distance_initial"));
                    bulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(17, "duration_sec_current"));
                    bulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(18, "duration_sec_initial"));
                    bulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(19, "rpe_current"));
                    bulkCopy.ColumnMappings.Add(new MySqlBulkCopyColumnMapping(20, "rpe_initial"));
                ...
                // Update the exisiting datas in tbl_HeavyApp
                foreach (var item in updates)
                {
                    await using var command = new MySqlCommand(DatabaseCommands.GetWorkoutUpdateSql(),
                                                            sqlServerConnection, (MySqlTransaction)transaction);

                    command.Parameters.AddWithValue("@PersonID_FK", personId);

                    #region ... rest of the add with value
                    ...
                    command.Parameters.AddWithValue("@ExerciseTitle", (object?)item.ExerciseTitle ?? DBNull.Value);
                    command.Parameters.AddWithValue("@SupersetId", (object?)item.SupersetId ?? DBNull.Value);
                    command.Parameters.AddWithValue("@ExerciseNotes", (object?)item.ExerciseNotes ?? DBNull.Value);

                    command.Parameters.AddWithValue("@SetIndex", (object?)item.SetIndex ?? DBNull.Value);
                    command.Parameters.AddWithValue("@SetType", (object?)item.SetType ?? DBNull.Value);

                    command.Parameters.AddWithValue("@WeightKg", (object?)item.WeightKg ?? DBNull.Value);
                    command.Parameters.AddWithValue("@Reps", (object?)item.Reps ?? DBNull.Value);
                    command.Parameters.AddWithValue("@DistanceKm", (object?)item.DistanceKm ?? DBNull.Value);
                    command.Parameters.AddWithValue("@DurationSeconds", (object?)item.DurationSeconds ?? DBNull.Value);
                    command.Parameters.AddWithValue("@Rpe", (object?)item.Rpe ?? DBNull.Value);
        ...
        #region Functions Get Datas from Datbase
        ...
        public async Task<int> GetPersonCountAsync()
        {
            ...
            {
                ErrorMessage = $"Error Get Person Count: {ex.Message}";
            }
        ...
        public async Task<List<GymWorkoutEntryModel>> GetHeavyAppWorkoutsAsync(int personId)
        {
            ...
            try
            {
                ...
                await using var SqlCommand = new MySqlCommand(DatabaseCommands.GetWorkoutEntriesSql(), sqlServerConnection);
        ...
        /// </summary>
        /// <remarks>Establishes a database sqlServerConnection, executes the step trend command retrieved from <see cref="DatabaseCommands.CmdGetSamsungHealthStepDailyTrend"/> with the given person identifier parameter, and reads the result set into a list of <see cref="StepDailyTrendChartModel"/> instances.</remarks>
        ...
        public async Task<List<StepDailyTrendChartModel>> GetStepTrendsAsync(int personId)
        {
            ...
            try
            {
                var SqlCommand = new MySqlCommand(DatabaseCommands.GetSamsungStepTrendSql(), sqlServerConnection);
