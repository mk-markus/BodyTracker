...
namespace BodyTracker.Services
{
    ...
    public partial class DatabaseService : ObservableObject, IDisposable
    {
        ...
        /// </summary>
        private bool serverConnectionOK = false;

        /// <summary>
        /// Gets or sets a value indicating whether the database server connection status is operational, sending a database connection state message via the messenger when the value changes.
        /// </summary>
        public bool ServerConnectionOK
        {
            get => serverConnectionOK;
            set
            {
                serverConnectionOK = value;
                WeakReferenceMessenger.Default.Send(new DatabaseConnectionStateMessage(serverConnectionOK));
            }
        }
        ...
        /// </summary>
        private string sqlServerFailureMessage = "";

        /// <summary>
        /// Gets or sets the SQL server failure message, sending a database error message via the messenger when the value changes.
        /// </summary>
        public string SqlServerFailureMessage
        {
            get => sqlServerFailureMessage;
            set
            {
                sqlServerFailureMessage = value;
                WeakReferenceMessenger.Default.Send(new DatabaseErrorMessage(sqlServerFailureMessage));
            }
        }

        /// <summary>
        /// Represents a cancellation token source used to safely signal and stop the background connection monitoring task.
        /// </summary>
        private readonly CancellationTokenSource connectionMonitorCts = new();

        /// <summary>
        /// Represents the background task responsible for periodically checking and maintaining the database server connection status.
        /// </summary>
        ...
        /// <summary>
        /// Represents the time interval used by the background monitor to periodically check the database connection health.
        /// </summary>
        private readonly TimeSpan monitorInterval = TimeSpan.FromSeconds(10);

        public string acutalDatabaseName { private set; get; } = string.Empty;

        public string actualDatabaseUser { private set; get; } = string.Empty;
        ...
        public DatabaseService(string sConnectionString)
        {
            ...
            connectionMonitorTask = Task.Run(() => MonitorConnectionLoopAsync(connectionMonitorCts.Token));
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                ...
                {
                    Debug.WriteLine($"Error during connection monitor shutdown: {ex.Message}");
        ...
        public async Task InitializeAsync()
        {
           await using var SqlServerConnection = await EtablishSqlServerConnection();
            ...
            SqlServerFailureMessage = string.Empty;
        }
        ...
        {
            sConnectionString = newConnectionString;
            // sofort prüfen
            _ = CheckAndReconnectAsync();
        }
        ...
        private async Task CheckAndReconnectAsync()
        {
            ...
            try
            {
                ...
                await EtablishSqlServerConnection();
                Debug.WriteLine("serverConnectionOK Status: " + ServerConnectionOK);
                if (!ServerConnectionOK)
                {
                    //SqlServerFailureMessage = "No SQL Server Connection";
                    
                    return;
                }
                //else
                //{
                //    SqlServerFailureMessage = "";
                //}
            ...
            {
                SqlServerFailureMessage = $"Error during connection check/reconnect: {ex.Message}";
            }
            finally
            {
        ...
        /// <returns></returns>
        private async Task MonitorConnectionLoopAsync(CancellationToken token)
        {
            try
            {
            ...
            {
                SqlServerFailureMessage = "Connection monitoring task was canceled.";
            }
        }
        ...
        public async Task<MySqlConnection> EtablishSqlServerConnection()
        {
            ...
            bool previousState = ServerConnectionOK; // Vorherigen Status merken
            ...
            try
            {
                ...
                acutalDatabaseName = sqlServerConnection.Database;

                using (var cmd = sqlServerConnection.CreateCommand())
                {
                    cmd.CommandText = "SELECT CURRENT_USER();";
                    actualDatabaseUser = cmd.ExecuteScalar()?.ToString();
                }

                ServerConnectionOK = sqlServerConnection.State == ConnectionState.Open;
                SqlServerFailureMessage = string.Empty;
            }
            catch (Exception ex)
            {

                SqlServerFailureMessage = $"Database connection error: {ex.Message}";
                ServerConnectionOK = false;

                sqlServerConnection.Dispose();
            }

            if (previousState != ServerConnectionOK)
            {
                WeakReferenceMessenger.Default.Send(new DatabaseConnectionStateMessage(ServerConnectionOK));
            }
        ...
        public async Task<int> CreatePersonAsync(string FirstName, string LastName, DateTime? BirthDate, double Height)
        {
            ...
            {
                SqlServerFailureMessage = $"Error creating person: {ex.Message}";
        ...
        public async Task<bool> IsMeasurementExistingAsync(DateTime dateToCheck, int PersonId)
        {
            ...
            {
                SqlServerFailureMessage = $"Error Checking Measurement Existing: {ex.Message}";
        ...
        {
            await using var sqlServerConnection = await EtablishSqlServerConnection();
            if (!ServerConnectionOK) return;
            try
            {
                TimeSpan timeWithoutMilliseconds = new TimeSpan(DateTime.Now.Hour, DateTime.Now.Minute, DateTime.Now.Second);
                DateTime finalDate = bodyMetricModel.MeasurementDate.Date.Add(timeWithoutMilliseconds);
                ...
                SqlCommand.Parameters.AddWithValue("@gw", (object?)bodyMetricModel.BodyWeight ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@bmi", (object?)bodyMetricModel.BMI ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@kf", (object?)bodyMetricModel.BodyFatPercentage ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@kfo", (object?)bodyMetricModel.BodyFatPercentageTop ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@kfu", (object?)bodyMetricModel.BodyFatPercentageBottom ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@mm", (object?)bodyMetricModel.BodyMusclePercentage ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@mmo", (object?)bodyMetricModel.BodyMusclePercentageTop ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@mmu", (object?)bodyMetricModel.BodyMusclePercentageBottom ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@kw", (object?)bodyMetricModel.BodyWaterPercentage ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@kk", (object?)bodyMetricModel.BodyBoneMass ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@vf", (object?)bodyMetricModel.BodyVisceralFat ?? 0.0f);

                await SqlCommand.ExecuteNonQueryAsync();
            }
            catch (Exception ex)
            {
                SqlServerFailureMessage = $"Error insert Body Metric: {ex.Message}";
        ...
        public async Task InsertBodyDimensionAsync(BodyDimensionsModel bodyDemensionModel)
        {
            ...
            {
                TimeSpan timeWithoutMilliseconds = new TimeSpan(DateTime.Now.Hour, DateTime.Now.Minute, DateTime.Now.Second);
                DateTime finalDate = bodyDemensionModel.MeasurementDate.Date.Add(timeWithoutMilliseconds);
                ...
                SqlCommand.Parameters.Add("@dt", (DbType)SqlDbType.DateTime2).Value = finalDate;
                SqlCommand.Parameters.AddWithValue("@br", (object?)bodyDemensionModel.ChestCircumference ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@ba", (object?)bodyDemensionModel.WaistCircumference ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@hu", (object?)bodyDemensionModel.HipsCircumference ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@fzbreast", (object?)bodyDemensionModel.FatTongBreastCrease ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@fzarmpit", (object?)bodyDemensionModel.FatTongArmpitCrease ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@fzabdomen", (object?)bodyDemensionModel.FatTongAbdominalCrease ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@fzhip", (object?)bodyDemensionModel.FatTongHipCrease ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@fzthigh", (object?)bodyDemensionModel.FatTongThighCrease ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@fzback", (object?)bodyDemensionModel.FatTongBackCrease ?? 0.0f);
                SqlCommand.Parameters.AddWithValue("@fztricep", (object?)bodyDemensionModel.FatTongTricepsCrease ?? 0.0f);
                await SqlCommand.ExecuteNonQueryAsync();

            }
            catch (Exception ex)
            {
                SqlServerFailureMessage = $"Error Insert Body Dimension: {ex.Message}";
            }
        }
        ...
        {
            await using var sqlServerConnection = await EtablishSqlServerConnection();
            if (!ServerConnectionOK) return false;
            ...
            {
                SqlServerFailureMessage = $"Error inserting Samsung Health food intake: {ex.Message}";
        ...
        public async Task<bool> InsertSamsungHealthStepDailyTrend(int PersonId, IEnumerable<StepDailyTrendModel> stepDailyTrendList, IProgress<double>? progress = null)
        {
            ...
            {
                SqlServerFailureMessage = $"Error inserting Samsung Health Daily Steps Trend: {ex.Message}";
                await tx.RollbackAsync();                
            }
                                                   ...
                                                   IProgress<double>? progress = null)
        {
            ...
            {
                SqlServerFailureMessage = $"Error inserting Heavy App workout data: {ex.Message}";
                await tx.RollbackAsync();
        ...
        public async Task<List<PersonModel>> GetPersonsAsync()
        {
            ...
            if (!ServerConnectionOK) return list;
            ...
            {
                SqlServerFailureMessage = $"Error Get Person: {ex.Message}";
            }
        ...
        public async Task<int> GetPersonCountAsync()
        {
            ...
            var sqlServerConnection = await EtablishSqlServerConnection();
            if (!ServerConnectionOK) return -1;
            ...
            {
                SqlServerFailureMessage = $"Error Get Person Count: {ex.Message}";
            }
        ...
        public async Task<BodyMetricModel?> GetLastBodyMetricAsync(int personId, DateTime today)
        {
            ...
            if (!ServerConnectionOK) return new BodyMetricModel();
            ...
            {
                SqlServerFailureMessage = $"Error Get Last Body Metrics: {ex.Message}";
            }
        ...
        public async Task<BodyDimensionsModel?> GetLastBodyDimensionsAsync(int personId, DateTime today)
        {
            ...
            if (!ServerConnectionOK) return new BodyDimensionsModel();
            ...
            {
                SqlServerFailureMessage = $"Error Get Body Demensions: {ex.Message}";
            }
        ...
        public async Task<List<FullBodyMeasurementDatasModel>> GetBodyMeasurementAsync(int personId, string sqlCommand)
        {
            ...
            {
                SqlServerFailureMessage = $"Error Get Body Measurement Async: {ex.Message}";
            }
        ...
        /// </returns>
        public async Task<List<GymWorkoutEntryModel>> GetHeavyAppWorkoutEntriesAsync(int personId)
        {
            ...
            try
            {
                ...
                while (await sqlDataReader.ReadAsync())
                {
                    ...
                    // Sichere Konvertierung für Weight (Spalte 2)
                    if (!sqlDataReader.IsDBNull(2))
                    {
                        var obj = sqlDataReader.GetValue(2);
                        entry.Weight = obj == null || obj is DBNull ? 0.0 : Convert.ToDouble(obj);
                    }

                    // Sichere Konvertierung für Reps (Spalte 3)
                    ...
                    // SetIndex (Spalte 4) als Ganzzahl
            ...
            {
                SqlServerFailureMessage = $"Error Get Get Heavy App Workout Entries Async: {ex.Message}";
            }
        ...
        /// </summary>
        /// <remarks>Establishes a database connection, executes the step trend command retrieved from <see cref="DatabaseCommands.CmdGetSamsungHealthStepDailyTrend"/> with the given person identifier parameter, and reads the result set into a list of <see cref="StepDailyTrendChartModel"/> instances.</remarks>
        /// <param name="personId">The unique identifier of the person whose step trends are being requested.</param>
        /// <returns>A task representing the asynchronous operation, containing a list of <see cref="StepDailyTrendChartModel"/> objects.</returns>
        public async Task<List<StepDailyTrendChartModel>> GetStepDailyTrendAsync(int personId)
        {
            ...
            if (!ServerConnectionOK) return result;

            try
            {
                var SqlCommand = new MySqlCommand(DatabaseCommands.CmdGetSamsungHealthStepDailyTrend(), sqlServerConnection);
            ...
            {
                SqlServerFailureMessage = $"Error Get Step Daily Trend Async: {ex.Message}";
            }
        ...
        public async Task DeleteBodyMetricAsync(int bodyMetricId)
        {
            ...
            {
                SqlServerFailureMessage = $"Error Delete Body Metric Async: {ex.Message}";
            }
        }
        ...
        public async Task DeleteBodyDimensionAsync(int bodyDimensionId)
        {
            ...
            {
                SqlServerFailureMessage = $"Error Delete Body Dimension Async: {ex.Message}";
            }
        }
            ...
            float? fatTongHipCrease, float? fatTongThighCrease, float? fatTongBackCrease, float? fatTongTricepsCrease)
        {
            ...
            {
                SqlServerFailureMessage = $"Error Update Measurement Async: {ex.Message}";
                await tx.RollbackAsync();
            }
        }
        ...
        public async Task UpdatePersonAsync(int PersonId, string firstName, string lastName, DateTime? birthDate, float height)
        {
            ...
            {
                SqlServerFailureMessage = $"Error Update Person Async: {ex.Message}";
                await tx.CommitAsync();
            }
        }
